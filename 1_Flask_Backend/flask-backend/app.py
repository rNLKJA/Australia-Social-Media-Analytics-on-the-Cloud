...
def create_choropleth_map_twitter():
    ...
    def ready_for_join(df, name):
        ...
        df["average_sentiment" + "(" + name + ")"] = (
            df["value.sum"] / df["count" + " " + name]
        ).round(2)

        return df

    # Load, normalize, and prepare each dataset
    for filename, topic in zip(
        ["crimeMentioned.json", "incomeMentioned.json", "sentimentLocation.json"],
        ["crime", "income", "sentiment"],
    ):
    ...
    sf_sal["geometry"] = sf_sal["geometry"].to_crs(
    ...
    merged = pd.merge(
        sf_sal,
        df_crime[["count crime", "average_sentiment(crime)", "SAL_CODE21"]],
        on="SAL_CODE21",
        how="left",
    )
    merged = pd.merge(
        merged,
        df_income[["count income", "average_sentiment(income)", "SAL_CODE21"]],
        on="SAL_CODE21",
        how="left",
    )
    merged = pd.merge(
        merged,
        df_sentiment[["count sentiment", "average_sentiment(sentiment)", "SAL_CODE21"]],
        on="SAL_CODE21",
        how="left",
    )
    ...
    columns = [
        ...
        "count income",
        "count sentiment",
        "average_sentiment(crime)",
        "average_sentiment(income)",
    ...
    for i, col in enumerate(plot):
        ...
        buttons.append(
            dict(
                ...
                args=[
                    {"visible": visible},
                    {"title": f"{col} Distribution Across Australia"},
                ],
            )
        )
...
def visualize_crime_data():
    ...
    summary = df_crime[columns_to_plot].describe(
    ...
    summary = df_crime[columns_of_offences].describe(
        percentiles=[0.0, 0.25, 0.5, 0.75, 1.0]
    )
    summary = summary.round(2)
    ...
    sf_lga["geometry"] = sf_lga["geometry"].to_crs(
        "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
    )
    ...
    for i, col in enumerate(columns_of_offences):
        ...
        buttons.append(
            dict(
                ...
                args=[
                    {"visible": visible},
                    {"title": f"{col} Distribution Across Melbourne LGAs"},
                ],
            )
        )
...
def visualize_income_data():
    ...
    summary = (
        df_vic_income[
            ["mean_aud", " median_aud", " sum_aud", " median_age_of_earners_years"]
        ]
    ...
    for i, col in enumerate(
        ...
        buttons.append(
            dict(
                ...
                args=[
                    {"visible": visible},
                    {"title": f"Personal {col} Distribution Across Australia SA2s"},
                ],
            )
        )
