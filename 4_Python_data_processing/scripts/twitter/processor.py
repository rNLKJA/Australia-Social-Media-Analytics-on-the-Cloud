...
def twitter_processor(file, cs, ce, sal_dict, rank, size, db, ns, sa, li):
    ...
    with open(
        ...
        with open(file, mode="rb") as f:
            ...
            progress_bar = tqdm(
                desc=f"{rank}: Processing tweets", unit="tweets", dynamic_ncols=True
            )
            ...
            while f.tell() < ce:
                ...
                tweet = Tweet(
                    tid=_id.group(1) if _id else NONE,
...
def twitter_processor_v1(file, cs, ce, sal_dict, rank, size, db, ns, sa, li):
